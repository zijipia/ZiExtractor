...
class ZiExtractor extends BaseExtractor {
	...
	async handlePlaylist(query, context) {
		...
		try {
			...
			playlist.tracks = playlistData.videos.map((video) =>
				this.createYTTrack(video, context, playlist),
			);
	...
	async handleVideo(query, context) {
		...
		try {
			...
			let video = await YouTubeSR.YouTube.getVideo(
	...
	async getRelatedTracks(track, history) {
		...
		try {
			...
			const uniqueTracks = result.filter(
				(video) => !history.tracks.some((track) => track.url === video.url),
			);
			this.log(`Found ${uniqueTracks.length} unique related tracks for track: "${track?.title}"`);

			const tracks = uniqueTracks.map((video) =>
				this.createYTTrack(video, { requestedBy: track.requestedBy }),
			);
