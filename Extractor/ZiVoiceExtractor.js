...
class ZiVoiceExtractor extends EventEmitter {
	...
	checkAudioQuality(pcmBuffer) {
		...
		const rms = Math.sqrt(
			int16Array.reduce((sum, sample) => sum + sample * sample, 0) / int16Array.length,
		);
    ...
    - First 10 values: ${monoBuffer.slice(0, 20).toString("hex")}`);
		...
		try {
			...
			if (!response.data || !response.data.result || response.data.result.length === 0) {
				this.debug(
					`No speech recognition results. Response data: ${JSON.stringify(response.data)}`,
				);
				return "";
			}
	...
	convertStereoToMono(stereoBuffer) {
		const stereoData = new Int16Array(
			stereoBuffer.buffer,
			stereoBuffer.byteOffset,
			stereoBuffer.length / 2,
		);
